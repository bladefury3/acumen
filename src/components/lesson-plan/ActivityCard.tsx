...
const ActivityCard: React.FC<ActivityCardProps> = ({
  ...
  return <Card className="h-full transition-all duration-300 hover:shadow-lg animate-fade-in">
      ...
      <CardContent className="space-y-4 pt-2">
        {activities.map((activity, idx) => <Card key={idx} className="border shadow-sm">
            ...
            <CardContent className="space-y-2 pt-2">
              <ul className="list-disc pl-4 space-y-2 marker:text-primary">
                {activity.details.map((step, stepIdx) => <li key={stepIdx} className="text-sm leading-relaxed text-muted-foreground hover:text-foreground transition-colors">
